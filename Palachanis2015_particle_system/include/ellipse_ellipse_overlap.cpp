                                ...
                                int *rtnCode)
{
    ...
    if ( (fabs (PHI_1) > (pi)) ) //WINNIE: because ellipses are symmetrical, it does not matter if we add pi to phi, that#s probably why fmod(PHI_1,pi) is used and not fmod(PHI_1,twopi)
    {
        PHI_1 = fmod (PHI_1, pi);
    }
    if ( (fabs (PHI_2) > (pi)) )
...
//-- for the two ellipses
    ...
    for (i = 0; i < nychk; i++)
    {
        ...
        if (fabs(ellipse2tr(x1, ychk[i], AA, BB, CC, DD, EE, FF)) < EPS*10.)//*10. quick fix by Winnie
        {
            nintpts++;
            if (DEBUG)
        ...
        if ((fabs(ellipse2tr(x2, ychk[i], AA, BB, CC, DD, EE, FF)) < EPS*10.)//*10 quick fix by Winnie
            && (fabs (x2 - x1) > EPS))
        {
            nintpts++;
            if (DEBUG)
...
//=========================================xs==============================
    ...
    case 0:
    case 1:
	// Winnie: we probably don't need this case because both our ellipses are always identical
	//printf("found 1 int point\n");
        //OverlapArea = nointpts (A1, B1, A2, B2, H1, K1, H2, K2, PHI_1,
         //                       PHI_2, H2_TR, K2_TR, AA,BB, CC, DD, EE,
        //7                      FF, rtnCode);
	//printf("OverlapArea: %f\n",OverlapArea);
        //free (ychk);
        //free(xint);
        //free(yint);
        //return OverlapArea;
                  ...
                  double EE, double FF, int *rtnCode)
{
    ...
    if (ellipse2tr (xmid, ymid, AA, BB, CC, DD, EE, FF) > EPS*10.) // changed from 0.0 to EPS*10. by Winnie
    {
        tmp = theta1;
        theta1 = theta2;
        theta2 = tmp;
	...
	printf("theta2=%f\n",theta2);
       ...
       // getc(stdin); // commented out by Winnie
    }
    ...
    //if(area1+area2>0.02)
    //{
//	printf("Twointpts: \t area1 =%f\n",area1);
//	printf("Twointpts: \t area2=%f\n",area2);
  //  }
    return area1 + area2;
}
             ...
             double CC, double DD, double EE, double FF)
{
    ...
    //fix 24.11.12
    eps_radian = 0.01; //arbitrary value
